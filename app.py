...
st.title("⚕️ Trợ Lý Dịch Thuật Y Khoa AI (Hoàn chỉnh)")
...
st.info("⚡ Chế độ tối ưu: Gộp đoạn (Batching) tăng tốc độ, bỏ qua chú thích ảnh (Caption), Text Box và Bảng biểu để giữ nguyên danh pháp gốc.")
...
if nut_dich and file_word is not None:
    try:
        ...
        if tong_so_doan == 0:
            st.warning("File Word không có đoạn văn bản thường nào để dịch (hoặc toàn bộ là bảng biểu/chú thích ảnh).")
        else:
            st.write(f"📂 Tìm thấy {tong_so_doan} đoạn văn bản chính. Đang tiến hành dịch...")
            ...
            st.download_button(
                ...
                file_name="Ban_Dich_Y_Khoa_Thanh_Cong.docx",
